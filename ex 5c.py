...
#flag flag
= 0
#appending dollar to end of input user_input =
user_input + "$"
stack = []
stack.append("$") stack.append(start_symbol)
input_len = len(user_input) index =
0
while len(stack) > 0:
#element at top of stack top
= stack[len(stack)-1]
print ("Top =>",top) #current
input current_input =
user_input[index]
...
key = top , current_input print
(key)
#top of stack terminal => not accepted if
key not in parsingTable:
flag = 1 break
value = parsingTable[key] if
value !='@':
value = value[::-1] value
= list(value)
#poping top of stack stack.pop()
else:
#push value chars to stack for
element in value:
stack.append(element)
stack.pop()
if flag == 0: print ("String
accepted!")
else:
...
(pd.DataFrame(new_table).fillna('-')) print ("\n")
return table
...
ans = follow(key, productions, ans) temp =
ans[key]
else:
ans[s] = ans[s].union(temp)
first_of_next = first(value[f+1:], productions) if
'@' in first_of_next:
if key!=s:
if key in ans:
temp = ans[key]
...
ans = ans.union('@')
f = first(st, productions) ans =
ans.union(x for x in f)
else: ans = ans.union(c)
return ans
if name ==" main ":
productions=dict()
grammar = open("grammar2", "r")
first_dict = dict() follow_dict = dict()
flag = 1 start = "" for
line in grammar:
l = re.split("( |->|\n|\||)*", line)
lhs = l[0] rhs = set(l[1:-1])-{''}
if flag : flag = 0 start = lhs
productions[lhs] = rhs
print ('\nFirst\n') for lhs in productions:
first_dict[lhs] = first(lhs, productions)
for f in first_dict: print (str(f) + " : " +
str(first_dict[f]))
print ("")
print ('\nFollow\n')
for lhs in productions:
...
for lhs in productions: follow_dict = follow(lhs, productions,
follow_dict)
for lhs in productions:
